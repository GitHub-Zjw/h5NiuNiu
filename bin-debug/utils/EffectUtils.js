...
(function (EffectUtils) {
    ...
    */
    ...
    /**
     * 显示对象蹲下跳起动画
     * @param downTime 蹲下动画时长(毫秒)
     * @param downStrength 下蹲强度(缩放大小)
     * @param jumpTime 弹跳动画时长
     * @param jumpStreingth 弹跳强度（像素）
     * @param backTime 下落时长
     */
    function jumpEffect(obj, downTime, downStrength, jumpTime, jumpStreingth, backTime) {
        if (downTime === void 0) { downTime = 100; }
        if (downStrength === void 0) { downStrength = 0.8; }
        if (jumpTime === void 0) { jumpTime = 100; }
        if (jumpStreingth === void 0) { jumpStreingth = 5; }
        if (backTime === void 0) { backTime = 100; }
        var orgineY = obj.y;
        egret.Tween.get(obj).to({ scaleY: downStrength }, downTime)
            .to({ y: obj.y - jumpStreingth, scaleY: 1 }, jumpTime)
            .to({ y: orgineY }, backTime);
    }
    EffectUtils.jumpEffect = jumpEffect;
